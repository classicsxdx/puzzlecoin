...
uint256 hashGenesisBlock("0x0000032ef603f7b8ff9c131089f43a8457d069a51e577ccc586c719901678575"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // puzzlecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "puzzlecoin Signed Message:\n";
...
{
    // puzzlecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY+9) - GetDepthInMainChain());
}
...
static const int64 nTargetTimespan = 60 * 60 * 24 * 100; // puzzlecoin: 100 day
static const int64 nTargetSpacing =  150; // puzzlecoin: 1 Minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // puzzlecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, puzzlecoin - DarkGravity v2, written by Evan Duffield - evan@puzzlecoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // puzzlecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // puzzlecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000032ef603f7b8ff9c131089f43a8457d069a51e577ccc586c719901678575");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Can you solve the puzzle? May 10 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xe40310e0d2b64274ec02ccf7af110dd10256d8c01f5a73f8c73336128a384011"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // puzzlecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("puzzlecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("puzzlecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static puzzlecoinMiner(CWallet *pwallet)
{
    printf("puzzlecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("puzzlecoin-miner");
    ...
    try { loop {
        ...
        printf("Running puzzlecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("puzzlecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&puzzlecoinMiner, pwallet));
}
